...
def portfolio_management_agent(state: AgentState):
    ...
    allow_short = state["metadata"].get("allow_short", True)  # 新增參數，預設為 True

    progress.update_status("portfolio_management_agent",
                           None, "Analyzing signals")
    ...
    for ticker in tickers:
        progress.update_status("portfolio_management_agent",
                               ticker, "Processing analyst signals")

        # Get position limits and current prices for the ticker
        risk_data = analyst_signals.get(
            "risk_management_agent", {}).get(ticker, {})
        position_limits[ticker] = risk_data.get("remaining_position_limit", 0)
        current_prices[ticker] = risk_data.get("current_price", 0)

        # Calculate maximum shares allowed based on position limit and price
        if current_prices[ticker] > 0:
            max_shares[ticker] = int(
                position_limits[ticker] / current_prices[ticker])
        else:
            max_shares[ticker] = 0
        ...
        for agent, signals in analyst_signals.items():
            if agent != "risk_management_agent" and ticker in signals:
                ticker_signals[agent] = {
                    "signal": signals[ticker]["signal"], "confidence": signals[ticker]["confidence"]}
        signals_by_ticker[ticker] = ticker_signals

    progress.update_status("portfolio_management_agent",
                           None, "Making trading decisions")

    # 根據 allow_short 參數選擇使用哪個決策函數
    if allow_short:
        ...
        result = generate_trading_decision_no_short(
            tickers=tickers,
            signals_by_ticker=signals_by_ticker,
            current_prices=current_prices,
            max_shares=max_shares,
    ...
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning({ticker: decision.model_dump(
        ) for ticker, decision in result.decisions.items()}, "Portfolio Management Agent")
...
def generate_trading_decision_no_short(
    tickers: list[str],
    signals_by_ticker: dict[str, dict],
    current_prices: dict[str, float],
    max_shares: dict[str, int],
    ...
    """Attempts to get a decision from the LLM with retry logic - no short selling version"""
    # Create the prompt template
    template = ChatPromptTemplate.from_messages(
        [
            (
