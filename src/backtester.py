...
class Backtester:
    def __init__(
        ...
        self.portfolio = {
            ...
            # The margin ratio required for shorts
    ...
    def analyze_performance(self):
        ...
        max_drawdown = getattr(
            self, 'performance_metrics', {}).get('max_drawdown')
        max_drawdown_date = getattr(
            self, 'performance_metrics', {}).get('max_drawdown_date')

        # If no value exists yet, calculate it
        if max_drawdown is None:
            rolling_max = performance_df["Portfolio Value"].cummax()
            drawdown = (
                performance_df["Portfolio Value"] - rolling_max) / rolling_max
            max_drawdown = drawdown.min() * 100
            max_drawdown_date = drawdown.idxmin().strftime(
                '%Y-%m-%d') if pd.notnull(drawdown.idxmin()) else None
